    ...
    return (
        <div className='text-sm border-[1px] border-dark_blue/20 rounded-md bg-white group'>
            <div className='relative group overflow-hidden bg-shop_light_bg'>
                ...
                {product?.images && <Image src={urlFor(product?.images[0]).url()} alt='ProductImage' loading='lazy' width={700} height={700}  className={`w-full h-64 object-contain overflow-hidden hoverEffect transition-transform bg-shop_light_bg duration-500 
              ${product?.stock !== 0 ? "group-hover:scale-105" : "opacity-50"}`}></Image>}
                         <AddWishlistButton product={product}></AddWishlistButton>
        {product?.status === "sale" ? (
